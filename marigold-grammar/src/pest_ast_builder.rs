...
impl PestAstBuilder {
    ...
    // Single codegen path: take_while is inherently sequential (reordering would
    // break prefix semantics), so no buffered/parallel variant is generated under
    // tokio/async-std, unlike build_filter_fn / build_map_fn.
