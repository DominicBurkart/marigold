...
pub use crate::ast::MarigoldProgram;
...
pub enum GrammarError {
    ParseError(pest::error::Error<crate::Rule>),
}

pub fn marigold_parse(input: &str) -> Result<ast::MarigoldProgram, GrammarError> {
    let parsed = PARSER::parse(crate::Rule::program, input)
        .map_err(GrammarError::ParseError)?
        .next()
        .unwrap();
...
fn parse_read_file(
    ...
    let mut rules = specific_input
        .into_inner()
        .into_iter()
        .collect::<Vec<iterators::Pair<Rule>>>();
    ...
    // Handle format and type_ident if needed
    if !unhandled_rules.is_empty() {
        return Err(GrammarError::ParseError(pest::error::Error::new_from_pos(
            pest::error::ErrorVariant::CustomError {
                message: format!("unhandled rule: {:?}", unhandled_rules),
...
mod tests {
    ...
    #[test]
    fn multiple_csv_files() {
        let parsed = marigold_parse(
            r#"
        read_file("test.csv", csv, struct=Foo)
            .ok_or_panic()
