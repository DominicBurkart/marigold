...
mod tests {
    ...
    async fn file_writer_write_flush_shutdown() {
        let path =
            std::env::temp_dir().join(format!("marigold_writer_test_{}.bin", std::process::id()));
        let file = tokio::fs::File::create(&path).await.unwrap();
        let mut w = Writer::file(file);
        w.write_all(b"marigold file test").await.unwrap();
        w.flush().await.unwrap();
        w.shutdown().await.unwrap();

        let content = tokio::fs::read(&path).await.unwrap();
        assert_eq!(content, b"marigold file test");
