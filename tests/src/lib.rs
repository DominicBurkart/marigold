...
mod tests {
    ...
    // Case 2: order is by stream position, not by value
    // (second stream's smaller values still appear after first stream's larger values)
    #[tokio::test]
    async fn test_chain_order_by_position_not_value() {
        let result = m!(
            range(5, 7)
                .chain(range(0, 2))
                .return
        ...
        .collect::<Vec<_>>()
        .await;
        assert_eq!(result, vec![5, 6, 0, 1]);
    }

    // Case 5: chained chains (a.chain(b).chain(c)) — exercises recursive grammar
    #[tokio::test]
    async fn test_chain_triple() {
        let result = m!(
            range(0, 2)
                .chain(range(10, 12))
                .chain(range(20, 22))
